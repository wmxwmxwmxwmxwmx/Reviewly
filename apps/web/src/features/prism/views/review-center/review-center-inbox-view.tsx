...
export function ReviewCenterInboxView({
  ...
  const { tasks, loading, error, allItems } = useReviewInbox({
    segment,
    reloadToken,
  })

  const unreadCount = allItems.filter((i) => i.attentionState === "unread").length
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-3">
      <div className="flex items-baseline justify-between gap-2 mb-2">
        <span className="text-xs text-muted-foreground font-mono">
          {loading ? "…" : `${tasks.length} 条${segment === "unread" ? "未查阅" : segment === "read" ? "已查阅" : ""}`}
          {!loading && unreadCount > 0 && segment !== "unread" ? (
            <span className="ml-2 text-ai-blue">{unreadCount} 未查阅</span>
          ) : null}
        </span>
      </div>
