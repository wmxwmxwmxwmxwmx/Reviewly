...
export function ReviewCenterStatsView() {
  ...
  return (
    <div className="p-4 sm:p-5 space-y-5">
      <div>
        <h2 className="text-sm font-semibold text-foreground">数据统计</h2>
        <p className="text-[11px] text-muted-foreground mt-0.5">评审效率与 AI 用量</p>
      </div>
      ...
      <section className="space-y-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          <StatTile label="本周分析" value={data.weeklyAnalysisCount} />
          <StatTile label="AI 调用" value={data.aiCalls} />
          <StatTile label="Token 消耗" value={data.totalTokens.toLocaleString()} />
          <StatTile label="AI 成本 (CNY)" value={`¥${totalCost.toFixed(2)}`} primary />
        </div>
      </section>

      <section className="rounded-lg border border-border bg-card p-3">
        <h3 className="text-xs font-semibold text-foreground mb-3">近 7 日分析趋势</h3>
        {trendEmpty ? (
